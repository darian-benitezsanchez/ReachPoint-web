import { getAllStudents, uniqueFieldsFromStudents, applyFilters, getStudentId, saveCampaign, toISODate } from '../data/campaignsData.js';
...
export function CreateCampaign(root) {
  ...
  function render() {
    ...
    if (step === 'filters') {
      page.append(
        ...
        row(
          select(fields, field, v=>{ field=v; hideSuggest(); }),
          select(['=','~','>','>=','<','<='], op, v=>{ op=v; })
        ),
        ...
        btn('Next: Optional call question','btn btn-primary',()=>{ if (!name.trim()) return; step='question'; render(); }),
      );
      ...
      page.append(
        ...
        collectResponses ? row(
          input({ placeholder:"Add another option (e.g., 'Left voicemail')", onkeydown:e=>{ if(e.key==='Enter'){ addOption(e.target.value); e.target.value=''; } } }),
          btn('Add','btn',()=>{ const el=page.querySelector('#addOpt'); })
        ) : null,
        btn('Next: Select reminder dates','btn btn-primary',()=>{ if(collectResponses && !questionText.trim()) return; step='dates'; render(); }),
  ...
  function valueWithSuggestions() {
    ...
    const inp = input({ placeholder:'value', oninput:e=>{ value=e.target.value; showSuggest(); }, onfocus:showSuggest, onblur:()=> setTimeout(hideSuggest,120) });
    inp.style.flex='1';
    const add = btn('Add','btn', ()=>{ if(!field || !op || !String(value).trim()) return; filters.push({ field, op, value }); value=''; render(); });
    const rowEl = row(inp, add);
    rowEl.style.marginBottom='8px';
    wrapDiv.append(rowEl);

    const sug = div('suggestWrap', suggestCard());
    suggestEl = sug; wrapDiv.append(sug);
    return wrapDiv;
  }
  function showSuggest() {
    ...
    suggestEl.innerHTML = ''; suggestEl.append(suggestCard(items));
  }
  function hideSuggest() { if (suggestEl) suggestEl.innerHTML=''; }
  function suggestCard(items=[]) {
    const card = div('suggestCard');
    const scroller = document.createElement('div'); scroller.style.maxHeight='240px'; scroller.style.overflow='auto';
  ...
  function buildValuesByField(rows, fields) {
    ...
    for (const r of rows) for (const f of fields) if (Object.prototype.hasOwnProperty.call(r,f)) {
      const v = r[f]; if (v!==undefined && v!==null) { const s = String(v).trim(); if (s) tmp.get(f).add(s); }
    }
  ...
  function chip(text, onX){ const c=div('chip', text); const x=document.createElement('span'); x.className='x'; x.textContent='×'; x.onclick=onX; c.append(x); return c; }
  ...
  function miniCalendar(months, selected, onToggle) {
    ...
    for (let m=0;m<months;m++){
      ...
      const header = div('calHeader', ...['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(t=>{ const s=document.createElement('div'); s.className='calHeadCell'; s.textContent=t; return s; }));
      const grid = div('calGrid', ...days.map((d,i)=>{
