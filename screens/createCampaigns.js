...
export function CreateCampaign(root) {
  ...
  /* ----- Suggestions (dropdown, live, keyboard) ----- */
  let suggestEl = null;
  ...
  function valueWithSuggestions() {
    const wrapDiv = div('suggest-host'); // position: relative

    const inp = input({
      placeholder: 'value',
      value: value,
      oninput: e => { value = e.target.value; buildAndShowSuggestions(); renderMatchCountOnly(); },
  ...
  function buildAndShowSuggestions() {
    ...
    let items = term ? pool.filter(v => v.toLowerCase().includes(term)) : pool.slice();
    ...
    if (items.length) {
      ...
      suggestEl.style.pointerEvents = 'auto';
    } else {
      suggestEl.style.display = 'none';
      suggestEl.style.pointerEvents = 'none';
    }
